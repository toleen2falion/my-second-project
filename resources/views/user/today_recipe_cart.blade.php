            ...
            <div class="table-responsive ">
              ...
              <thead>
                ...
                <form action="{{ route('checkout') }}" method="post" enctype="multipart/form-data"
                    ...
                    $j = 1; 
                    @endphp
                    @if(session('today_recipe_cart'))
                        @foreach(session('today_recipe_cart') as $id => $details)
                        ...
                        <!-- /// -->
                            ...
                            <tr data-id="{{ $id }}">
                            <td>{{ $j }}</td>
                                <td>  <input type="hidden"  name="inputs[{{$i}}][ingredient_id]" value="{{ $id }}" class="form-control  today_recipe_cart_update"  /> 
                                </td>
                                                   
                                <td data-th="Product">
                                     <input type="text" name="inputs[{{$i}}][name]" value="{{ $details['name'] }}" class="form-control  today_recipe_cart_update"  /> 
                                ...
                                ?>
                                <td class="actions m-3" data-th="">
                                    <button class="btn btn-danger btn-sm today_recipe_cart_remove">  <i class="fa-solid fa-trash-can"></i></button>
                                </td>
                            </tr>
                            @php
                             $i++;
                ...
                <tfoot>
                    ...
                    <tr>
                        <td colspan="5" class="text-right">
                            <a href="{{ url('/home/today_recipe') }}" class="btn btn-danger"> <i class="fa fa-arrow-left"></i>  إضافة المزيد</a>
                            <button class="btn btn-success"><i class="fa fa-money"></i> حفظ</button>
                        </td>
                    </tr>
...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
    ...
    $(".today_recipe_cart_remove").click(function (e) {
        e.preventDefault();
        ...
        if(confirm("Do you really want to remove?")) {
            $.ajax({
                url: '{{ route('remove_from_today_recipe_cart') }}',
                method: "DELETE",
                data: {
                    _token: '{{ csrf_token() }}', 
